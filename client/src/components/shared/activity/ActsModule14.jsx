...
import qr from "../../../assets/optional_acts/main/module14/qr.png"
import imgs from "../../../assets/optional_acts/main/module14/img.png"
import img from "../../../assets/optional_acts/main/module14/given.png"

import axios from 'axios'


export const ActsModule14 = () => {
    ...
    const save_act = (act_no) => {
        ...
        if (filedata != null) {
            ...
            axios.post("http://localhost:8000/api/activities/", formData, {
                ...
                setLoading(false)
                if (response.data.act_no == 27) {
                    setSubmitted(true)
                }
                if (response.data.act_no == 28) {
                    setSubmitted2(true)
                }
                setFileData(null)
            }).catch((err) => {
  ...
  return (
    <div className='max-w-screen-2xl mx-auto'>
        ...
        <p className='text-base text-center  text-blue-gray-800 tracking-wide leading-7'><span className='font-bold text-lg'>Directions: </span>On a sheet of paper, copy and match each expression in Column A with its
simplified product in Column B. Take a photo of it and upload it to the submission bin in
JPEG, PNG, or PDF file format.</p>
       ...
       <form encType='multipart/form-data' className='w-1/2 grid grid-cols-1 gap-7 mx-auto py-8'>
            ...
            <Button type='button' onClick={() => save_act(27)} disabled={submitted} loading={loading}>Submit</Button>
       </form>
        ...
        <div className='grid grid-cols-1 gap-5 items-center justify-center justify-items-center mb-8'>
            ...
            <a target='_blank' className='bg-green-700 text-white px-8 py-2 rounded drop-shadow hover:bg-white hover:text-green-700 border-4 border-green-700 transition-all' href="https://view.genial.ly/659bd34028310900141330ca/interactive-content-special-product-patterns-darts-quiz">Click to Play</a>
            <img src={qr} alt={qr} />
        </div>
        ...
        <form encType='multipart/form-data' className='w-1/2 grid grid-cols-1 gap-7 mx-auto py-8'>
            ...
            <Button type='button' onClick={() => save_act(28)} disabled={submitted2} loading={loading}>Submit</Button>
       </form>
