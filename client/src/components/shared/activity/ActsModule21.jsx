...
import imgs from "../../../assets/optional_acts/main/module21/img.png"
import img from "../../../assets/optional_acts/main/module21/given.png"

import axios from 'axios'


export const ActsModule21 = () => {
    ...
    const save_act = (act_no) => {
        ...
        if (filedata != null) {
            ...
            axios.post("http://localhost:8000/api/activities/", formData, {
                ...
                setLoading(false)
                if (response.data.act_no == 41) {
                    setSubmitted(true)
                }
                if (response.data.act_no == 42) {
                    setSubmitted2(true)
                }
                setFileData(null)
            }).catch((err) => {
  ...
  return (
    <div className='max-w-screen-2xl mx-auto'>
        ...
        <p className='text-base text-center  text-blue-gray-800 tracking-wide leading-7'><span className='font-bold text-lg'>Directions: </span>Directions: Identify the mistake in each expression. Correct it by showing the complete
solution. Be attentive to common mistakes related to special product patterns. Upload in
a jpeg, png, or pdf file formats.</p>
       ...
       <form encType='multipart/form-data' className='w-1/2 grid grid-cols-1 gap-7 mx-auto py-8'>
            ...
            <Button type='button' onClick={() => save_act(41)} disabled={submitted} loading={loading}>Submit</Button>
       </form>
        ...
        <div className='grid grid-cols-1 gap-5 items-center justify-center justify-items-center mb-8'>
            ...
            <a target='_blank' className='bg-green-700 text-white px-8 py-2 rounded drop-shadow hover:bg-white hover:text-green-700 border-4 border-green-700 transition-all' href="https://app.nearpod.com/presentation?pin=RU4DV">Click to Play</a>
        ...
        <form encType='multipart/form-data' className='w-1/2 grid grid-cols-1 gap-7 mx-auto py-8'>
            ...
            <Button type='button' onClick={() => save_act(42)} disabled={submitted2} loading={loading}>Submit</Button>
       </form>
