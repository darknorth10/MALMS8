...
export const PreTest = () => {
  ...
  const [message2, setMessage2] = useState("Welcome Grade 8 Learner! Let's start navigating your Personalized Learning Journey!")
  ...
  useEffect(() => {
    ...
    if (parseFloat(mastery) > 1 ) {
      redirect('/classroom')
    }
    axios.get("https://malms8.pythonanywhere.com/api/questions/1/assessment/", {
      headers: {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      setTimeout(() => {
        ...
        setGreet("Great work!")
        setMessage("You have finished the pre test")
        setOpen(true)
      ...
      axios.patch(`http://localhost:8000/api/users/${id}/`, { mastery: mastery, pretest: points }, {
        headers: {
          Authorization: "Token " + token,
        }
      }).then((response) => {
